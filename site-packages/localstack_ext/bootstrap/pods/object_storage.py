...
class ObjectStorageProvider(abc.ABC):
	...
	@abc.abstractmethod
	def get_available_versions(self):0
	@abc.abstractmethod
	def get_current_active_version_number(self):0
	@property
...
class ObjectStorageLocal(ObjectStorageProvider):
	...
	def get_available_versions(B):
		C=[];A=Version.DEFAULT_INITIAL_VERSION_NUMBER
		while True:
			if not B.version_exists(A):return C
			D=B.get_version(A)
			if D.finalized:C.append(A)
			A+=1
	def get_current_active_version_number(B):
		C=B.get_available_versions();A=C[-1]if C else Version.DEFAULT_INITIAL_VERSION_NUMBER
		if B.version_exists(A+1):return A+1
		return A
