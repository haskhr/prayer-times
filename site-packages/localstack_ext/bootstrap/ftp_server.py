...
def do_start_ftp(port,asynchronous=_A):
	B=port;LOG.info('Starting (S)FTP server on port %s...'%B);apply_patches();C=DummyAuthorizer();F=new_tmp_dir();C.add_user(ROOT_USER[0],ROOT_USER[1],F,perm=FTP_USER_PERMISSIONS);G=new_tmp_dir();C.add_anonymous(G);A=TLS_FTPHandler;H,I,I=create_ssl_cert();A.certfile=H;A.authorizer=C;A.passive_ports=FTP_PASSIVE_PORTS;A.masquerade_address=ext_config.LOCALHOST_IP
	def D(*E):
