...
class RestartLocalstack(Item):
	...
	@contextmanager
	def _cloudpods_restore(self):from localstack_ext.bootstrap.pods_client import CloudPodsLocalService as C;D=os.path.abspath(persistence_session.localstack_session.volume_dir);A=f"file:{D}/test-pod-{short_uid()}.zip";B=C();B.export_pod(target=A);yield;B.import_pod(source=A)
class LocalstackSession:
	def __init__(A,persistence_mode,port=4566):A.port=port;A.volume_dir=tempfile.mkdtemp(prefix='localstack-pytest-');A.persistence_mode=persistence_mode;A.server=_C;A.env_vars={}
	def start(A,timeout):
		B=timeout
		if A.server:return
