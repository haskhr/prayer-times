...
class StoreSerializationCheckerPlugin:
	@pytest.hookimpl()
	def pytest_configure(self,config):from localstack.aws.handlers import serve_custom_service_request_handlers as A;from localstack_persistence.pickling import reducers as B;B.register();A.append(get_dirty_marker_handler());config.addinivalue_line('markers','skip_store_check(reason=None): skip the store serialization check')
