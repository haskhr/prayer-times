...
class SparkInstaller(ArchiveDownloadAndExtractInstaller):
	...
	def _post_process(A,target):B=target;C=A._get_install_dir(B);E=A._get_install_marker_path(C);D=f"/usr/local/spark-{A.version}";F=os.path.join(D,'bin',_D);copy_local_into_external(C,E,D,F);post_process_spark(A,B)
...
def post_process_spark(installer=_B,target=_B):
...
def post_process_spark_mono_container(installer,target=_B):
	A=installer
	if not ext_config.BIGDATA_MONO_CONTAINER:return
	install_hadoop_for_spark(A,target);install_awsglue_local(A);patch_additional_jar_files(A);patch_spark_python_dependencies(A)
	if A.version.startswith('2.'):from localstack_ext.packages.java import java_package as B;B.get_installer('8').install()
...
def install_hadoop_for_spark(spark_installer,target=_B):
...
def patch_additional_jar_files(installer):
	B=installer;D=B.get_installed_dir();download_additional_jar_files(B.version);C=f"{D}/conf/spark-defaults.conf";A=load_file(C)
	if'/gluePyspark'not in A:A+=f"\nspark.driver.extraClassPath {spark_jar_lib_location()}/*\nspark.executor.extraClassPath {spark_jar_lib_location()}/*\nspark.driver.allowMultipleContexts = true\n";save_file(C,A)
def patch_spark_python_dependencies(installer):C=installer.get_installed_dir();A=os.path.join(C,'python/lib/py4j-*-src.zip');A=glob.glob(A)[0];B={'from collections import':'from collections.abc import'};replace_in_zip_file(A,'py4j/java_collections.py',B);A=os.path.join(C,'python/lib/pyspark.zip');B={'import collections\n':'import collections.abc\n','collections.Iterable':'collections.abc.Iterable'};replace_in_zip_file(A,'pyspark/resultiterable.py',B);B={'obj.co_argcount, obj.co_kwonlyargcount':'obj.co_argcount, obj.co_posonlyargcount, obj.co_kwonlyargcount','co.co_argcount,\n            co.co_kwonlyargcount,':'co.co_argcount, co.co_posonlyargcount, co.co_kwonlyargcount,'};replace_in_zip_file(A,'pyspark/cloudpickle.py',B)
def replace_in_zip_file(zip_file,file_path,search_replace,raise_if_missing=_C):
...
def download_additional_jar_files(spark_version):
	A=SPARK_JAR_URLS['all']+SPARK_JAR_URLS.get(spark_version,[])
	for B in A:download_and_cache_jar_file(B)
...
def download_and_cache_jar_file(jar_url):
	...
	cp_r(A,C);return A
def spark_jar_lib_location(cache=_C):
	if ext_config.BIGDATA_MONO_CONTAINER:spark_package.install();A=spark_package.get_installed_dir();B=os.path.dirname(os.path.dirname(A));return os.path.join(B,'gluePyspark'if not cache else'bigdata-jars')
